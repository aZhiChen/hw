...
vector<string> code;
vector<string> res(32);//记录寄存器是否被占用
int ResTmp = 8;//$8~$25为用于保存临时数据的寄存器
    ...
    for(int i = 0; i < QUA.size(); i++){
        ...
        if( s == "main,-,-,-" )
            ...
            for(int j = 2; j < s.size(); j++){
                if(s[j] == ',') ist1 = false;
                if(s[j-1] == ',' && ist1 == false) ist2 = true;
                if(ist1 == true)
                    t1 += s[j];
                if(ist2 == true)
                    t2 += s[j];            
            }
            bool isid1 = false;//第一个数据是否是变量
            bool isid2 = false;//第二个数据是否是变量
            isid1 = isID(t1);
            isid2 = isID(t2);
            if( isid2 == false ){
                cout << "error in generate code in =" << endl;
                exit(EXIT_FAILURE);
            }
            string myres1 = "";
            bool myres1isfound = false;
            //先找已存在的寄存器
            for(int k = 8; k < 25; k++){
                if( res[k] == t1 ){
                    myres1 = "$" + to_string(k);
            ...
            if( isid1 ){//加载第一个变量到寄存器
                string mycode = "lw " + myres1 + ", &" + t1;
                code.push_back(mycode);
            }
            else{
                string mycode = "lui " + myres1 + ", " + t1;
                code.push_back(mycode); 
            }
            code.push_back(mycode);
            mycode = "st " + myres1 + ", &" + t1;
            ...
            for(int j = 2; j < s.size(); j++){
                if(s[j] == ',') ist1 = false;
                if(s[j-1] == ',' && ist1 == false) ist2 = true;
                if( s[j] == ',' && ist1 == false && ist2 == true ) ist2 = false;
                if( s[j-1] == ',' && ist1 == false && ist2 == false ) ist3 = true;
                if(ist1 == true)
                    t1 += s[j];
                if(ist2 == true)
                    t2 += s[j];            
            ...
            string mycode1 =  "lw " + myres1 + ", ";
            string mycode2 = "lw " + myres1 + ", "; 
            if( isid1 ) mycode1 += "&";
            if( isid2 ) mycode2 += "&";
            mycode1 += t1;
            mycode2 += t2;
            ...
            mycode3 += myres1 + ", " + myres2 + ", " + myres1;
            string mycode4 = "st " + myres1 + ", &" + t3;
            code.push_back(mycode1);
            code.push_back(mycode2);
            code.push_back(mycode3);
            code.push_back(mycode4);
        }
        else if( s[0] == '>' || s[0] == '<' || (s[0] == '=' && s[1] == '=') || (s[0] == '!' && s[1] == '=')  ){
            string t1, t2, t3;
            t1 = t2 = t3 = "";
            bool ist1=true;
            bool ist2 = false;
