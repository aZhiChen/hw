...
output为输出的矩阵
*/
static __global__ void cuda_v2(
    ...
    if( idy < height && idx < width ){
        ...
        __shared__ double logcons[26]; 
        //用线程初始化
        if( threadIdx.y == 0 && threadIdx.x < 26)
            logcons[threadIdx.x] = threadIdx.x == 0 ? 0 : log((double)threadIdx.x);
        __syncthreads();//需要控制线程同步
