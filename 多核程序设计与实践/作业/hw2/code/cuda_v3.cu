...
static __device__ double logcons[26];
static __global__ void cuda_v3(
    ...
    if( idy < height && idx < width ){
        ...
        int n = (min(2,idx) + 1 + min(2,width-idx)) * (min(2,idy) + 1 + min(2, height-idy));
        double n_div = 1/n;
        double ans = logcons[n]; 
        for( int i = 0; i < 16; i++ ){
            if( cnt[i] )
                ans -= logcons[cnt[i]] * cnt[i] * n_div;
        }
...
int main(int argc, char* argv[]){
    ...
    CHECK(cudaMemcpyToSymbol(logcons, (const double*)logcons_h,sizeof(logcons_h)));
