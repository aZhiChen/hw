...
def laplacian_matrix(adj_matrix):
    ...
    adj_matrix is a csr_matrix
    '''
    A = np.array(adj_matrix.todense())
    degree = np.sum(A, axis=1) 
    A = -A
    for i in range(len(A)):
        A[i][i] = A[i][i] + degree[i]  
    return A


def max_idx(degree_list, n):
    '''
        degree_list: list of node degree
        n: the largest n degree node index 
    '''
    max_number = heapq.nlargest(n, degree_list) 
    max_index = []
    for t in max_number:
        index = degree_list.index(t)
