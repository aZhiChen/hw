...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        help="informations from teacher model want to distill to student model",
    )
...
def run(args):
    ...
    if args.exp_setting == "tran":
        ...
        h_list, out, score_val, score_test = run_transductive(
            conf,
            model,
            g,
            feats,
        ...
        h_list, out, score_val, score_test_tran, score_test_ind = run_inductive(
            conf,
            model,
            g,
            feats,
    ...
    h_np = h_list.detach().cpu().numpy()
    np.savez(output_dir.joinpath("out"), out_np)
    np.savez(output_dir.joinpath("hidden"),h_np)
