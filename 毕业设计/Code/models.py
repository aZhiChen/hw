...
class Model(nn.Module):
    ...
    def __init__(self, conf): 
        ...
        self.which = conf["which"]

        #weight of Egvectors
        if self.which == "pos":
            #self.params = torch.nn.Parameter(torch.randn(200).to(conf["device"]).requires_grad_())
            self.params = torch.nn.Parameter(torch.randn(conf["vec_num"]).to(conf["device"]).requires_grad_())
            #self.params = torch.nn.Parameter(torch.ones(1,200, requires_grad = True))
    ...
    def forward(self, data, feats, vectors = None, idx = None):
        """
        data: a graph `g` or a `dataloader` of blocks
        """
        if self.which == "pos":
            #self.params = nn.Softmax(self.params)
            with open("C:\\Users\\azhi\\Desktop\\hw\\Graduation_project\\myCode\\outputs\\models.params.txt", "a") as f:
                f.write(str(self.params))

            if idx == None:
                vectors = self.params * vectors
            else:
                vectors = self.params * vectors[idx]
            feats = torch.cat((feats, vectors), 1)    
        if "MLP" in self.model_name:
            return self.encoder(feats)
        else:
            return self.encoder(data, feats)

    def forward_fitnet(self, data, feats, vectors = None, idx = None):
        """
    ...
    def inference(self, data, feats, vectors = None, idx = None):
        if "SAGE" in self.model_name:
            if self.which == "pos":
            #model.params = nn.Softmax(model.params)
                if idx == None:
                    vectors = self.params * vectors
                else:
                    vectors = self.params * vectors[idx]
            ...
            return self.forward(data, feats, vectors, idx)
