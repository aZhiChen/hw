...
def load_cpf_data(dataset, dataset_path, seed, labelrate_train, labelrate_val):
    ...
    vectors = readEvc(dataset, dataset_path)
    Idx = readIdx(dataset, dataset_path)
    return vectors,g, labels, idx_train, idx_val, idx_test, Idx
...
def load_hidden_t(hidden_t_dir):
    return torch.from_numpy(np.load(hidden_t_dir.joinpath("hidden.npz"))["arr_0"])
...
class SparseGraph:
    ...
    # ?: why largest_connected_components
...
def readEvc(dataset, dataset_path):
    data_path = Path.cwd().joinpath(dataset_path, f"{dataset}_egvs")
    f = open(data_path,"r") 
...
def readIdx(dataset, dataset_path):
    data_path = Path.cwd().joinpath(dataset_path, f"{dataset}_MaxDegreeIdx")
    f = open(data_path,"r") 
    lines = f.readlines()      #读取全部内容 ，并以列表方式返回
    Idx = []
    for line in lines:
        ...
        Idx = []
        for num in line.split(","):
            Idx.append(torch.tensor(int(num)))
    f.close()
